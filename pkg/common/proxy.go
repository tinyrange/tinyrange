...
func Proxy(srvConn, cliConn connLikeObject) error {
	// channels to wait on the close event for each connection
	serverClosed := make(chan struct{}, 1)
	clientClosed := make(chan struct{}, 1)
	errC := make(chan error, 1)

	go broker(srvConn, cliConn, clientClosed, errC)
	go broker(cliConn, srvConn, serverClosed, errC)
...
func broker(dst, src connLikeObject, srcClosed chan struct{}, errC chan error) {
