...
func (parser *packageCollection) Dependencies() ([]common.BuildDefinition, error) {
	panic("unimplemented on packageCollection")
}

// Build implements common.BuildDefinition.
func (parser *packageCollection) Build(ctx common.BuildContext) error {
	panic("unimplemented on packageCollection")
}
...
func (parser *packageCollection) NeedsBuild(ctx common.BuildContext) (bool, error) {
	panic("unimplemented on packageCollection")
}
...
func (parser *packageCollection) load(ctx common.MinimalBuildContext) error {
	...
	ctx.Logger().Debug("built all package sources", "took", time.Since(start))
	start = time.Now()
	...
	case err := <-errors:
		return err
	case <-done:
		ctx.Logger().Debug("loaded all packages", "count", len(records), "took", time.Since(start))
...
func (parser *packageCollection) InstallerFor(c common.MinimalBuildContext, pkg *common.Package, tags common.TagList) (*common.Installer, error) {
	...
	if err != nil {
		if sErr, ok := err.(*starlark.EvalError); ok {
			c.Logger().Error("got starlark error", "error", sErr, "backtrace", sErr.Backtrace())
		}
