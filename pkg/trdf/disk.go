...
const maxVirtualBlock = 4 * 1024 * 1024 * 1024

const trdfMagic = "TRDF"
const trdfVersion = 1
...
func (s *superblock) create() {
	copy(s[:4], trdfMagic)
	binary.LittleEndian.PutUint32(s[4:8], trdfVersion)
}

func (s *superblock) validate() error {
	if string(s[:4]) != trdfMagic {
		return fmt.Errorf("invalid magic: %s", s[:4])
	}

	if binary.LittleEndian.Uint32(s[4:8]) != trdfVersion {
...
/*
Disk is a simple disk format that is backed by a single file.
...
func (t *Disk) Close() error {
	if err := t.Flush(); err != nil {
		return fmt.Errorf("Close: %w", err)
	}

	if c, ok := t.file.(io.Closer); ok {
		return c.Close()
	}
...
func (t *Disk) offsetToVirtualBlockAddress(offset int64) (virtualBlockAddress, int64) {
	return virtualBlockAddress(offset / blockSize), offset % blockSize
}

func (t *Disk) underlyingSize() (int64, error) {
	fi, err := t.file.Stat()
	if err != nil {
...
func (t *Disk) readDirectBlock(p []byte, block fileBlockAddress) error {
	n, err := t.file.ReadAt(p, int64(block)*blockSize)
	if err == io.EOF {
		return err
	} else if err != nil {
...
func (t *Disk) writeDirectBlock(p []byte, block fileBlockAddress) error {
	n, err := t.file.WriteAt(p, int64(block)*blockSize)
	if err == io.EOF {
		return err
	} else if err != nil {
...
func (t *Disk) readIndirectFromCache(block fileBlockAddress) (*indirectBlock, error) {
...
func (t *Disk) writeIndirectToCache(p *indirectBlock) {
	t.blockCache[p.offset] = p
	t.pendingWrites[p.offset] = struct{}{}
}

func (t *Disk) allocateBlock() (fileBlockAddress, error) {
	block := t.lastBlockWritten + 1
	t.lastBlockWritten = block

	// write a block of zeros to the file.
	p := make([]byte, blockSize)
	if err := t.writeDirectBlock(p, block); err != nil {
		return fileBlockAddress(0), fmt.Errorf("allocateBlock(%d): %w", block, err)
	}

	return block, nil
}

func (t *Disk) virtualToIndirectOffsets(block virtualBlockAddress) (first int, second int, third int, fourth int) {
...
func (t *Disk) virtualToFileBlockAddress(block virtualBlockAddress) (fileBlockAddress, error) {
	if block > maxVirtualBlock {
	...
	if fourthBlock == 0 {
		...
		t.writeIndirectToCache(third)
	}

	return fourthBlock, nil
}

func (t *Disk) readBlock(p []byte, block virtualBlockAddress) error {
	if len(p) != blockSize {
...
func (t *Disk) writeBlock(p []byte, block virtualBlockAddress) error {
	if len(p) != blockSize {
...
func (t *Disk) ReadAt(p []byte, off int64) (n int, err error) {
	...
	if startOffset == 0 && endOffset == 0 {
		// The read is for a series of blocks.
		x := 0
		for i := start; i < end; i++ {
			if err := t.readBlock(p[x*blockSize:(x+1)*blockSize], i); err != nil {
				return -1, fmt.Errorf("ReadAt(%d, %d): %w", len(p), off, err)
			}
...
func (t *Disk) WriteAt(p []byte, off int64) (n int, err error) {
	start, startOffset := t.offsetToVirtualBlockAddress(off)
	end, endOffset := t.offsetToVirtualBlockAddress(off + int64(len(p)))
	...
	if startOffset == 0 && endOffset == 0 {
		...
		x := 0
		for i := start; i < end; i++ {
			if err := t.writeBlock(p[x*blockSize:(x+1)*blockSize], i); err != nil {
				return -1, fmt.Errorf("ReadAt(%d, %d): %w", len(p), off, err)
			}
...
func (t *Disk) Size() int64 {
	return t.superBlock.diskSize()
}

func (t *Disk) Resize(size int64) error {
	return fmt.Errorf("Resize(%d) not implemented", size)
}

func (t *Disk) Flush() error {
...
var (
	_ vm.MemoryRegion = &Disk{}
	_ io.Closer       = &Disk{}
)

func OpenDisk(file UnderlyingFile, size int64) (*Disk, error) {
	f := &Disk{
		file:          file,
	...
	f.lastBlockWritten = fileBlockAddress(underlyingSize / blockSize)
