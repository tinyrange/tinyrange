...
// ---- Test stream fakes ----
type readStream struct{ tb testing.TB; bufs [][]byte }
...
func TestOpenFileWriteReadClose(t *testing.T) {
    ...
    if err != nil { t.Fatalf("OpenFile write: %v", err) }
    ...
    if err := s.Write(ws); err != nil { t.Fatalf("Write: %v", err) }
    if !ws.closed { t.Fatalf("Write stream not closed") }
    ...
    if err != nil { t.Fatalf("OpenFile read: %v", err) }
    ...
    for _, b := range rs.bufs { got.Write(b) }
...
func TestExecNonTTYCapturesStdoutAndExit(t *testing.T) {
    s := NewLocalMachineServer()
    shell := "/bin/sh"
    if runtime.GOOS == "windows" { t.Skip("skip on windows") }
    cmd := "-c"
    ...
    for _, b := range rs.bufs { out.Write(b) }
    if out.String() != "hello" { t.Fatalf("unexpected stdout %q", out.String()) }

    // Wait should return 0
    wr, err := s.WaitProcess(context.Background(), &machinepb.WaitProcessRequest{Pid: of.Pid})
    if err != nil { t.Fatalf("WaitProcess: %v", err) }
    if wr.ExitCode != 0 { t.Fatalf("unexpected exit code %d", wr.ExitCode) }
}
...
func TestExecTTYCombinesStdoutAndStderr(t *testing.T) {
    ...
    if err != nil { t.Fatalf("Exec tty: %v", err) }
    if of.StderrFd != 0 { t.Fatalf("expected no stderr fd in TTY mode, got %d", of.StderrFd) }
    rs := &readStream{tb: t}
    ...
    var out strings.Builder
    for _, b := range rs.bufs { out.Write(b) }
    if out.String() != "outerr" { t.Fatalf("unexpected combined %q", out.String()) }
...
func TestMkdirStatChmod(t *testing.T) {
    ...
    if err != nil || !st.Exists || !st.IsDir { t.Fatalf("Stat dir: %+v err=%v", st, err) }

    // non-existent
    st2, err := s.Stat(context.Background(), &machinepb.StatRequest{Path: filepath.Join(dir, "nope"), FollowSymlinks: true})
    if err != nil || st2.Exists { t.Fatalf("Stat nonexist: %+v err=%v", st2, err) }
    ...
    if st3.Mode&0600 != 0600 { t.Fatalf("expected mode to include 0600, got %o", st3.Mode) }
}

func TestStatSymlinkFollowBehavior(t *testing.T) {
    if runtime.GOOS == "windows" { t.Skip("skip symlink tests on windows") }
    s := NewLocalMachineServer()
    dir := t.TempDir()

    // Create a file and a symlink to it
    target := filepath.Join(dir, "target.txt")
    if err := os.WriteFile(target, []byte("x"), 0644); err != nil { t.Fatalf("write target: %v", err) }
    link := filepath.Join(dir, "link.txt")
    if err := os.Symlink(target, link); err != nil { t.Fatalf("symlink: %v", err) }
    ...
    if err != nil { t.Fatalf("lstat symlink: %v", err) }
    if !st1.Exists || st1.IsDir { t.Fatalf("unexpected lstat: %+v", st1) }
    ...
    if err != nil { t.Fatalf("stat symlink: %v", err) }
    if !st2.Exists || st2.IsDir { t.Fatalf("unexpected stat: %+v", st2) }
    if st2.Size != 1 { t.Fatalf("expected size 1, got %d", st2.Size) }

    // Directory target
    tdir := filepath.Join(dir, "d")
    if err := os.MkdirAll(tdir, 0755); err != nil { t.Fatalf("mkdir: %v", err) }
    dlink := filepath.Join(dir, "dlink")
    if err := os.Symlink(tdir, dlink); err != nil { t.Fatalf("symlink dir: %v", err) }

    st3, err := s.Stat(context.Background(), &machinepb.StatRequest{Path: dlink, FollowSymlinks: false})
    if err != nil { t.Fatalf("lstat dlink: %v", err) }
    if !st3.Exists || st3.IsDir { t.Fatalf("unexpected lstat dir link: %+v", st3) }
    st4, err := s.Stat(context.Background(), &machinepb.StatRequest{Path: dlink, FollowSymlinks: true})
    if err != nil { t.Fatalf("stat dlink: %v", err) }
    if !st4.Exists || !st4.IsDir { t.Fatalf("unexpected stat dir link: %+v", st4) }

    // Broken link
    broken := filepath.Join(dir, "broken")
    if err := os.Symlink(filepath.Join(dir, "no_such"), broken); err != nil { t.Fatalf("symlink broken: %v", err) }
    st5, err := s.Stat(context.Background(), &machinepb.StatRequest{Path: broken, FollowSymlinks: false})
    if err != nil || !st5.Exists { t.Fatalf("lstat broken: %+v err=%v", st5, err) }
    st6, err := s.Stat(context.Background(), &machinepb.StatRequest{Path: broken, FollowSymlinks: true})
    if err != nil || st6.Exists { t.Fatalf("stat broken: %+v err=%v", st6, err) }
}
...
func TestChownSameIDsNoop(t *testing.T) {
    ...
    if err := os.WriteFile(p, []byte("x"), 0644); err != nil { t.Fatalf("write: %v", err) }
    ...
    if err != nil {
        if resp != nil && resp.Success { t.Fatalf("chown returned error but success=true") }
        // EPERM or EINVAL acceptable on restricted environments
        t.Logf("chown not permitted: %v", err)
    } else {
        if resp == nil || !resp.Success { t.Fatalf("expected success from chown") }
    }
}
