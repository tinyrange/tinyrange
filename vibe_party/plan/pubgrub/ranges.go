...
func (s singletonRange) Intersect(other Range, cmp Comparator) Range {
    if other.Contains(s.V, cmp) { return s }
    return emptyRange{}
}
...
func (r intervalRange) Contains(v Version, cmp Comparator) bool {
    if r.Lower != nil {
        ...
        if c < 0 || (c == 0 && !r.IncLower) { return false }
    }
    if r.Upper != nil {
        c := cmp.Compare(v, r.Upper)
        if c > 0 || (c == 0 && !r.IncUpper) { return false }
    }
    return true
}
func (r intervalRange) Intersect(o Range, cmp Comparator) Range {
    ...
    case anyRange:
        return r
    case singletonRange:
        if r.Contains(oo.V, cmp) { return oo }
        return emptyRange{}
    case intervalRange:
        ...
        if lo != nil && hi != nil {
            c := cmp.Compare(lo, hi)
            if c > 0 || (c == 0 && (!incLo || !incHi)) { return emptyRange{} }
        }
...
func (r intervalRange) String() string {
    var lo, hi string
    if r.Lower != nil { if r.IncLower { lo = "[" + r.Lower.String() } else { lo = "(" + r.Lower.String() } } else { lo = "(-∞" }
    if r.Upper != nil { if r.IncUpper { hi = r.Upper.String() + "]" } else { hi = r.Upper.String() + ")" } } else { hi = "+∞)" }
    return lo + "," + hi
}
...
func (n negatedRange) Intersect(o Range, cmp Comparator) Range {
    ...
    case emptyRange:
        return emptyRange{}
    case singletonRange:
        if n.Disallow.Contains(oo.V, cmp) { return emptyRange{} }
        return oo
    case anyRange:
        return n
    default:
        inter := oo.Intersect(n.Disallow, cmp)
        if !inter.IsEmpty() && rangeString(inter) == rangeString(oo) { return emptyRange{} }
        return o
    }
}
...
func (u unionRange) Contains(v Version, cmp Comparator) bool { return u.A.Contains(v, cmp) || u.B.Contains(v, cmp) }
func (u unionRange) Intersect(o Range, cmp Comparator) Range {
    switch oo := o.(type) {
    case emptyRange:
        return emptyRange{}
    case singletonRange:
        if u.Contains(oo.V, cmp) { return emptyRange{} }
        return oo
    case anyRange:
        return u
    default:
        ...
        if (!ia.IsEmpty() && rangeString(ia) == rangeString(oo)) || (!ib.IsEmpty() && rangeString(ib) == rangeString(oo)) { return emptyRange{} }
        return o
    }
}
...
func filterByRange(vers []Version, r Range, cmp Comparator) []Version {
    ...
    for _, v := range vers { if r.Contains(v, cmp) { out = append(out, v) } }
    return out
}
