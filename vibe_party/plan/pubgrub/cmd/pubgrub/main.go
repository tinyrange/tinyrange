...
func (m *memProvider) IsYanked(pkg pg.PackageID, ver pg.Version) bool {
...
func demoUniverse() *memProvider {
	...
	add := func(p string, versions ...string) {
		for _, v := range versions {
			m.versions[pg.PackageID(p)] = append(m.versions[pg.PackageID(p)], parseSemver(v))
		}
	}
	dep := func(p, v string, cons ...pg.Constraint) {
		if m.deps[pg.PackageID(p)] == nil {
			m.deps[pg.PackageID(p)] = map[string][]pg.Constraint{}
		}
