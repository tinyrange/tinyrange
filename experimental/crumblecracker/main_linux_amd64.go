...
func cpuSetRegisters(cpu *KVMCPU) error {
	sregs, err := cpu.GetSpecialRegisters()
	if err != nil {
	...
	if err := cpu.SetSpecialRegisters(sregs); err != nil {
		return fmt.Errorf("failed to set special registers: %w", err)
	}
	...
	if err := cpu.SetRegisters(regs); err != nil {
		return fmt.Errorf("failed to set registers: %w", err)
	}
...
func cpuSetCPUID(kvm *KVMDevice, cpu *KVMCPU) error {
	supported, err := kvm.GetSupportedCPUID()
	if err != nil {
	...
	for i := 0; i < int(supported.NumEnt); i++ {
		...
		if ent.Function == KVM_CPUID_SIGNATURE {
			ent.EAX = KVM_CPUID_FEATURES
	...
	if err := cpu.SetCPUID2(supported); err != nil {
		return fmt.Errorf("failed to set CPUID: %w", err)
	}
...
func appMain() error {
	...
	if err := vm.SetIdentityMapAddr(0xffffc000); err != nil {
		return fmt.Errorf("failed to set identity map address: %w", err)
	}

	if err := vm.SetTSSAddr(0xffffd000); err != nil {
		return fmt.Errorf("failed to set TSS address: %w", err)
	}
	...
	if err := vm.CreatePIT2(false); err != nil {
		return fmt.Errorf("failed to create PIT2: %w", err)
	}

	// Map 1GB of memory.
	mem, err := vm.MapMemory(0, 1<<30)
	if err != nil {
	...
	if err := cpuSetRegisters(cpu); err != nil {
		return fmt.Errorf("failed to set registers: %w", err)
	}

	if err := cpuSetCPUID(dev, cpu); err != nil {
		return fmt.Errorf("failed to set CPUID: %w", err)
	}
