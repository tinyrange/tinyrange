	...
	for _, member := range decl.Members {
		...
		case *StructOrUnionMember:
			...
			if member.Name != "_" {
				...
				// Set Method
				setReceiver := pointerTo(structType)
				if isDynamic {
...
func (g *SysIL4Generator) WriteTo(w io.Writer) (int64, error) {
	var buf bytes.Buffer
	if _, err := fmt.Fprintf(&buf, "// generated by structGen. DO NOT EDIT MANUALLY\n\n"); err != nil {
		return 0, err
	}
	...
	if err := format.Node(&buf, goToken.NewFileSet(), &ast.File{
